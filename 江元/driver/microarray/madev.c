...
extern int compatible_finger;

int is_microarray_fp_flag = 0;
extern int is_fps_fp_flag;
...
int init_connect(void){
    ...
    printk("wangzhongping: fingerpirnt microarray don't connect.\n");
    return 0;   
}
...
int mas_probe(struct spi_device *spi) {
	...
	if(1 == compatible_finger)
		{
			 MALOGF("compatible_finger");
		return -1;
	}
/*
	if(1 == is_fps_fp_flag)	
	{		
		printk("[mas_probe] fingerprint is fps958e, return -1!!!\n");		
		return -1;	
	}
*/
    MALOGF("start");	
...
#endif	
//	is_microarray_fp_flag = 1; 
//  fingerprint_name_info = "a116n";
