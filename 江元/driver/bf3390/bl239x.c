...
static int bl229x_read_chipid1(struct bl229x_data *bl229x)
{
	...
	}else if(id == 0x5383)//3390
	{
		...
		SENSOR_DEFAULT_CONTRAST_V3 = 0xB9;
...
static DEVICE_ATTR(image,0444,bl229x_show_image,NULL);
...
static DEVICE_ATTR(init,0444,bl229x_show_init,NULL);
...
static DEVICE_ATTR(reset,0444,bl229x_show_reset,NULL);
...
{	
	...
	int err = 0;
	if (1 == compatible_finger)
	{	BTL_DEBUG("compatible_finger");
		return -1;
	}
	BTL_DEBUG("  ++\n\n");
	...
	mtspi_set_dma_en(0);
   	...
   	if(bl229x_read_chipid1(bl229x) != 0x5383 && bl229x_read_chipid1(bl229x) != 0x5283)
   	{
		...
		if(bl229x_read_chipid2(bl229x) != 0x5183)
		{
			...
			printk("wangzhongping: fingerprint bf3390 don't connect.\n");
	...
	err = request_threaded_irq(spi->irq_num, NULL, bl229x_eint_handler, IRQ_TYPE_EDGE_RISING | IRQF_ONESHOT, SPI_DRV_NAME, bl229x);
#endif
...
//不使用spi child node方式注册设备master->dev.of_node = pdev->dev.of_node;  child node
