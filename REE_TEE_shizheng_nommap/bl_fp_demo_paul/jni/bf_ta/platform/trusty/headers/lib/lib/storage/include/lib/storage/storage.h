...
/**
 * storage_rename_file() - rename then file.
 * @session: the storage_session_t returned from a call to storage_open_session
 * @old_file_name: the name of the file to rename
 * @new_file_name: the new name of the file
 *
 * Return: NO_ERROR on success, negative error code on failure.
 */
int storage_rename_file(storage_session_t session, const char *old_file_name,
                        const char *new_file_name,uint32_t opflags);
