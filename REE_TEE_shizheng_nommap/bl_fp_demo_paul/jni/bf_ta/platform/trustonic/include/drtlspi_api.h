...
/**call case: Mtp_SpiOpenSession -> Mtp_drSpiSend -> Mtp_drSpiSend ...
	-> Mtp_drSpiSend->... -> Mtp_drSpiSend->Mtp_SpiCloseSession
a.only use for MTP read.write!.  
b.Mtp_SpiOpenSession() .Mtp_SpiCloseSession(), must appear in pairs!.
********/

#ifndef SPI_PIN_SWITCH_MODE
_TLAPI_EXTERN_C tlApiResult_t drSpiSend1(const void *tx_buf, void	*rx_buf, uint32_t len, struct mt_chip_conf *chip_conf, uint32_t flag);
#else
_TLAPI_EXTERN_C tlApiResult_t drSpiSend1(struct spi_transfer *transfer_data );
#endif

#ifndef SPI_PIN_SWITCH_MODE
_TLAPI_EXTERN_C tlApiResult_t Mtp_drSpiSend(const void *tx_buf, void	*rx_buf, uint32_t len, struct mt_chip_conf *chip_conf, uint32_t flag);
#else
_TLAPI_EXTERN_C tlApiResult_t Mtp_drSpiSend(struct spi_transfer *transfer_data );
#endif
 ...
 * @return  session id
 */
_TLAPI_EXTERN_C uint32_t Mtp_SpiOpenSession();
 ...
 */
_TLAPI_EXTERN_C tlApiResult_t Mtp_SpiCloseSession();
