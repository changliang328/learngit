...
#define BL229X_GET_IMAGEDATA	        _IOWR(BL229X_IOCTL_MAGIC_NO, 15, u32)
#define BL229X_NAV_REPORT          		_IOWR(BL229X_IOCTL_MAGIC_NO, 16, u32)
...
#ifndef CHIP_BF3182
#define SENSOR_DEFAULT_GAIN_V3		   (0x3f)//(0xB5)	
#define SENSOR_CAPTURE_GAIN_V3		   (0x3f)//(0x37)	
...
static DECLARE_WAIT_QUEUE_HEAD(navdata_waiter);

u8 imagebuf[FINGER_WIDTH*FINGER_HEIGHT*2 + 1];
...
static int bl229x_create_inputdev(void)
{
    ...
    set_bit(EV_ABS,bl229x_inputdev->evbit);
    set_bit(ABS_X,bl229x_inputdev->absbit);	
    set_bit(ABS_Y,bl229x_inputdev->absbit);
	set_bit(ABS_PRESSURE,bl229x_inputdev->absbit);

	set_bit(BTN_TOUCH,bl229x_inputdev->keybit);
	set_bit(INPUT_PROP_DIRECT, bl229x_inputdev->propbit);

	set_bit(ABS_MT_TRACKING_ID,bl229x_inputdev->absbit);


	set_bit(ABS_MT_TOUCH_MAJOR, bl229x_inputdev->absbit);
	set_bit(ABS_MT_TOUCH_MINOR, bl229x_inputdev->absbit);
    set_bit(ABS_MT_POSITION_X,bl229x_inputdev->absbit);	
    set_bit(ABS_MT_POSITION_Y,bl229x_inputdev->absbit);

	
	input_set_abs_params(bl229x_inputdev, ABS_MT_POSITION_X, 0, ABS_X_MAX, 0, 0);
	input_set_abs_params(bl229x_inputdev, ABS_MT_POSITION_Y, 0, ABS_Y_MAX, 0, 0);
...
int isEmptyImage(struct bl229x_data *bl229x)
{		

	unsigned int i = 0,  j = 0, ret = 0;	
	u8 tmp = 0;	u8 *buf = bl229x->image_buf;	
	for(i = 0, j = FINGER_WIDTH*FINGER_HEIGHT; i < FINGER_WIDTH*FINGER_HEIGHT; i++)	
	{	
	...
	BTL_DEBUG("efficient points num = %d", j);
	if(j < 200)	
	{
		ret = 1;	
	}
...
irqreturn_t bl229x_eint_handler(int irq,void *data)
{
	u8 intStatus = 0, isSecondImage = 1;
	int ret = 0;
	int curStatus = 0;
	   ...
	   if (g_bl229x->report_key != 0)
	   	{
			...
			ret = isEmptyImage(g_bl229x);			
			if(!ret)
			{
				memcpy(&imagebuf[0], g_bl229x->image_buf, FINGER_WIDTH*FINGER_HEIGHT);
			}
		   while(ret == 0)			
		   {	
				if(atomic_read(&is_busy) == 1){
					memset(imagebuf, 0xff, FINGER_WIDTH*FINGER_HEIGHT*2+1);
					break;			
				}else{
/*				
					...
					*/
						...
						isSecondImage = 0;	
						memcpy(&imagebuf[FINGER_WIDTH*FINGER_HEIGHT], g_bl229x->image_buf, FINGER_WIDTH*FINGER_HEIGHT);
		   ...
		   navdata_sta = 1;
		   wake_up_interruptible(&navdata_waiter);
...
static long bl229x_ioctl(struct file *filp, unsigned int cmd,unsigned long arg)
{
    ...
    case BL229X_GETIMAGE:
		...
		memset(g_bl229x->image_buf,0x00,READIMAGE_BUF_SIZE);//must be 0  , for read directly ,no exchange
		//add end
	...
	case BL229X_INIT_ARGS:
		...
		break;
	case BL229X_GET_IMAGEDATA:
		BTL_DEBUG("BL229X_GET_IMAGEDATA \n");
		if((tmp = wait_event_interruptible_timeout(navdata_waiter, navdata_sta != 0, msecs_to_jiffies(800))) == 0)
			imagebuf[FINGER_WIDTH*FINGER_HEIGHT*2] = 1;
		else
			imagebuf[FINGER_WIDTH*FINGER_HEIGHT*2] = 0;
		navdata_sta = 0;
		BTL_DEBUG("after wait_event_interruptible_timeout %d\n", tmp);
		if (copy_to_user((void __user*)argp,imagebuf,sizeof(u8)*(FINGER_WIDTH*FINGER_HEIGHT*2 + 1)) != 0 ) {
               error = -EFAULT;
        }
		memset(imagebuf, 0xff, sizeof(u8)*(FINGER_WIDTH*FINGER_HEIGHT*2 + 1));
		break;
	case BL229X_NAV_REPORT:
		copy_from_user(dataBuf,(void __user*)arg,32);
		BTL_DEBUG("BL229X_NAV_REPORT %d\n", ((u32 *)dataBuf)[0]);
		input_report_key(bl229x_inputdev,((u32 *)dataBuf)[0] ,1);			
		input_sync(bl229x_inputdev);
		input_report_key(bl229x_inputdev,((u32 *)dataBuf)[0] ,0);			
		input_sync(bl229x_inputdev);
		break;
    default:
        error = -ENOTTY;
...
static ssize_t bl229x_read(struct file *file, char  *buff,size_t count, loff_t *ppos)
{
	...
	memset(g_bl229x->image_buf, 0xff, READIMAGE_BUF_SIZE);
