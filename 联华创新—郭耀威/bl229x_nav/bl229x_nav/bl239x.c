...
#ifdef CHIP_BF3182
#define BL229X_HEIGHT  128
#define BL229X_WIDTH   72
#else
...
#define BL229X_GET_IRQSTA	            _IOWR(BL229X_IOCTL_MAGIC_NO, 13, u32)
...
static struct fingerprintd_params_t fingerprintdParams = {
  ...
  .def_enroll_try_times = 8,
  .def_match_try_times = 8,
...
static int spi_read_frame(struct bl229x_data *bl229x)
{
    ...
    //hexdump(imagerxpix,BL229X_WIDTH);
...
int bl229x_dev_inactive_driver_init(struct bl229x_data *bl229x)
{
	...
	bl229x_spi_write_reg(REGA_FINGER_DT_INTERVAL_HIGH, 0x02);
...
// R0D0 - IC revision 0, driver edition 0
	...
	//	bit[5] 0, AGC disable
	//  bit[6] (default 0), ADC reverse, 1’b0 -> ADC输入按位取反； 1’b1->ADC输入不取反；
	bl229x_spi_write_reg(0x10, 0x4C);
...
static long bl229x_ioctl(struct file *filp, unsigned int cmd,unsigned long arg)
{
	...
	case BL229X_INIT_ARGS:
		...
		break;
	case BL229X_GET_IRQSTA:
		{
			BTL_DEBUG("BL229X_GET_IRQSTA \n");
           if (copy_to_user((void __user*)argp,&irq_sta,sizeof(u8)*1) != 0 ) {
               error = -EFAULT;
           }
    	}
    default:
