...
static char nav_status = 0;
static char nav_sta[100] = {' '};
...
int g_fid_index = 0;
extern int btl_nav_core_init(void); //davie
extern int btl_nav_core_ctl_init(void);
//extern fingerprint_var_nav_t    gFingerprintVarNav;
...
static int32_t btl_api_waitSignal(int32_t timeout_sec,int32_t timeout_ms)
{
    ...
    ret = 0;
    signal(SIGIO, &btl_api_responseFunc);//btl_api_responseFunc;btl_nav_core_wakeNavThread
...
int btl_nav_getBackLightSta(void)
{
	...
	LOGD("lcd-backlight/brightness: %s ",  buf);
	close(tempFd);
...
static void *thEnrollFunc(void *arg)
{
    ...
    LOGD("++ %s", __func__);
	property_get("persist.blestech.nav", nav_sta, NULL);	//
	LOGD("persist.blestech.nav %s", nav_sta);
...
int32_t Fp_InitLock()
{
	...
	//memcpy(nav_sta, "enable", 7);
	LOGD("ret = %d, persist.blestech.nav %s", ret, nav_sta);
	if(strncmp(nav_sta, "enable", 6) == 0)
	{
		btl_nav_core_init();
		#ifdef USE_PIPE_CONN
		btl_nav_core_ctl_init();
		#else
		#endif
		nav_status = 1;
	}
