...
*/
	...
	LOGD(TAG,"xy_sum_sq = %d, center_x_sq = %d, center_y_sq = %d!\n", xy_sum_sq, center_x_sq, center_y_sq);
	if(((right_x_sq + left_x_sq + down_x_sq + up_x_sq) - (right_y_sq + left_y_sq + down_y_sq + up_y_sq) >= 30) && (xy_sum_sq >= 0)/*(((right_x_sq >= 10) || (left_x_sq >= 10)) && ((down_x_sq >= 5) || (up_x_sq >= 5)))*/)
	{
		if(((down_x_sq >= 30) || (up_x_sq >= 30)))// && ((right_x_sq >= 5) || (left_x_sq >= 5))
		//if((right_x_sq + left_x_sq) > 40 || (down_x_sq + up_x_sq) > 50)
		{
			if(x_pos >= 2 && (((down_x_sq + up_x_sq) > 50)))//(right_x_sq + left_x_sq) > 40) ||
			{
				LOGD(TAG, "up gDirection moving !\n");
				gDirection = DIR_RIGHT; 
			}
			else if(x_neg >= 3 && (right_x_sq + left_x_sq) > 50)
			{
	...
	if((gDirection == ACT_NONE) && ((right_x_sq + left_x_sq + down_x_sq + up_x_sq) - (right_y_sq + left_y_sq + down_y_sq + up_y_sq) < 30) && (xy_sum_sq >= 0)) //&& ((right_y_sq >= 15) || (left_y_sq >= 15)) &&  ((down_y_sq >= 40) || (up_y_sq >= 40)))
	{
		if(((right_y_sq >= 40) || (left_y_sq >= 40)) && ((down_y_sq + up_y_sq >= 30)))//((down_y_sq >= 40) || (up_y_sq >= 40)))// 
		//if((right_y_sq + left_y_sq) > 60 || (down_y_sq + up_y_sq) > 60)
		{
			if(y_pos >= 3 && (((right_y_sq + left_y_sq) > 80) || (down_y_sq + up_y_sq) > 80))//|| (down_y_sq + up_y_sq) > 45
			{
				LOGD(TAG, "right gDirection moving !\n");
				gDirection = DIR_DOWN;		
			
			}
			else if(y_neg >= 3 && (((right_y_sq + left_y_sq) > 80) || (down_y_sq + up_y_sq) > 80))
			{
	...
	else if((interval_time > 120) && (interval_time < 500))
	{
		
		right_x_sq > 12 ? tmp++ : tmp;
		left_x_sq > 12 ? tmp++ : tmp;
		down_x_sq > 12 ? tmp++ : tmp;
		up_x_sq > 12 ? tmp++ : tmp;

		right_y_sq > 12 ? tmp++ : tmp;
		left_y_sq > 12 ? tmp++ : tmp;
		down_y_sq > 21 ? tmp++ : tmp;
		up_y_sq > 12 ? tmp++ : tmp;

		if(tmp <= 2 && xy_sum_sq < 25)
		{
			LOGD(TAG, "CLK_SINGLE\n");
