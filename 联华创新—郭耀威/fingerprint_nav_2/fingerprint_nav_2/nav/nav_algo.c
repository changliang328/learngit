...
/*
	...
	else if(((right_x_sq + left_x_sq + down_x_sq + up_x_sq) < (right_y_sq + left_y_sq + down_y_sq + up_y_sq)) && (((right_y_sq >= 15) || (left_y_sq >= 15)) &&  ((down_y_sq >= 40) || (up_y_sq >= 40))))
	{
		...
		{
			LOGD("down gDirection moving !\n");
			gDirection = DIR_DOWN;		//down
		}
		else if(info_start->down.y >= info_end->down.y && info_start->up.y >= info_end->up.y)
		{
			LOGD("up gDirection moving !\n");
			gDirection = DIR_UP;		//up
		}
		else
			LOGD("y gDirection move invalid !\n");
	}
*/	

	if(((right_x_sq + left_x_sq + down_x_sq + up_x_sq) - (right_y_sq + left_y_sq + down_y_sq + up_y_sq) >= -50) && (xy_sum_sq >= 0)/*(((right_x_sq >= 10) || (left_x_sq >= 10)) && ((down_x_sq >= 5) || (up_x_sq >= 5)))*/)
	{
	...
	{
		if(((right_y_sq >= 40) || (left_y_sq >= 40)) && ((down_y_sq >= 40) || (up_y_sq >= 40)))// 
...
char cal_touch_area(int	fd)
{
	char sta1 = 0 ,sta2 = 0;
	//int i = 0;
	unsigned int frame = 1;
	char tmp = 0; 
	unsigned char imageA[112*96];
