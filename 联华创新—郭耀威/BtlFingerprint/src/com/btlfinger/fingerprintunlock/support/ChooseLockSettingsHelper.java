...
public class ChooseLockSettingsHelper {
	...
	private boolean confirmPassword(int paramInt,
		...
		if (!this.mLockPatternUtils.isLockPasswordEnabled(ActivityManager.getCurrentUser())) {
			//this.mActivity.finish();
			//((LockUI) mActivity).susecessAndExit();
			//this.mActivity.onActivityResult(100,Activity.RESULT_OK,null);
	...
	private boolean confirmPattern(int paramInt,
		...
		if ((!this.mLockPatternUtils.isLockPatternEnabled(ActivityManager.getCurrentUser()))) {
			((LockUI) mActivity).susecessAndExit();
			return false;
		}
		Intent localIntent = new Intent();
	...
	boolean launchConfirmationActivity(int paramInt,
		...
		Log.i("aaa",
				"launchConfirmationActivity: "
						+ mLockPatternUtils.getKeyguardStoredPasswordQuality(ActivityManager.getCurrentUser()));
		switch (this.mLockPatternUtils.getKeyguardStoredPasswordQuality(ActivityManager.getCurrentUser())) {
