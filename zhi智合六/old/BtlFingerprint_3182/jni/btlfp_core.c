...
)
{
    ...
    //curContrast = g_best_capture_contrast;
    ...
    while (matchCompleted == 0) {
        ...
        if (btl_preprocess_imageCheckEmpty(imageA,white_value_empty_image,points_threshold_empty_image,RAW_FINGERPRINT_SIZE) == -1) {
            isValidFpImage = 0;
            LOGD("empty counts:%d",emptyImageCounts);
            if(pressed == 0) {
                emptyImageCounts++;
                if (emptyImageCounts >= g_capture_ck_empty_frames)
                    return -ERR_EMPTY;
                continue;
            }
        } else
        ...
        if(isValidFpImage) {
			...
			ret = FP_ImagePreproc(pFpImage->p_image, FINGER_WIDTH, FINGER_HEIGHT, 253, 50);
			if(ret == 0)
			{
				#if (SAVE_IMAGE)
				char imagename[256];
			...
			REG_VALUE_3290 oldRegValue = {g_def_gain,0x48,curContrast};
			REG_VALUE_3290 newRegValue;
			#ifndef CHIP_BF3182
			ret = bl_AutoGainDacp_3290(imageA, FINGER_WIDTH, FINGER_HEIGHT,  &oldRegValue, &newRegValue,128);
			#else
			ret = bl_AutoGainDacp_3182(imageA, FINGER_WIDTH, FINGER_HEIGHT,  &oldRegValue, &newRegValue,128);
			#endif
			curContrast = newRegValue.reg_value_0x1b;
			LOGD ("ret=%d mybestContrast=%x oldGainDacp.dacp=%x newGainDacp.dacp=%x",ret, mybestContrast,oldRegValue.reg_value_0x1b, newRegValue.reg_value_0x1b);
        }
...
int32_t btl_core_isFingerUp ()
{
    ...
    curContrast =  g_def_capture_contrast;//g_best_fingerup_contrast;

    while (g_CancelFlag != 1 && timeoutCouns < maxTimeOuts) {
        btl_core_readFpImage(image,100,g_fd,curContrast); //??ȡͼ??g_points_threshold
        imageCheckRet = btl_preprocess_imageCheckEmpty(image,WHITE_POINT_VALUE,2000,RAW_FINGERPRINT_SIZE);
    	LOGD("fingerup flag:%d,%d WHITE_POINT_VALUE=%d",fingerUpFlag,g_points_threshold,WHITE_POINT_VALUE);
        timeoutCouns++;
        if (imageCheckRet == -1) {
...
int btl_core_mmiTest()
{
    ...
    int ret=0;
    char imagename[200];
    uint8_t t_image[RAW_FINGERPRINT_SIZE]= {0x00};
    fp_image_t fpImage;
    uint8_t tryTimes = 1;
