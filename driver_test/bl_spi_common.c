...
int bl_read_chipid(struct bl_fingerprint_data *bl_data)
{
	...
	if((ret == BL_CHIPID_3182) ||(ret == BL_CHIPID_3390) || (ret == BL_CHIPID_3590))
	{
...
{
	free(bl_data->tx_buf);
	BF_LOG("private=%d",bl_data->private);

	if(NULL != bl_data->private)
	{
		free(bl_data->chip_params->params);	
		free(bl_data->chip_params);
	}
