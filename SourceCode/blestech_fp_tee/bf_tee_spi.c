...
#ifdef NEED_INPUT_EVENT_OPT
			input_report_key(bf_dev->fp_inputdev,KEY_HOMEPAGE ,1);
			input_sync(bf_dev->fp_inputdev);
			input_report_key(bf_dev->fp_inputdev,KEY_HOMEPAGE ,0);
			input_sync(bf_dev->fp_inputdev);
#endif
			break;
...
int bl_sysfs_init(struct bf_device *bf_dev)
{
    ...
    if(ret) {
        BF_LOG("%s sysfs_create_file failed\n",__func__);
    }
    return ret;
}
...
#else
	bf_dev->tx_buf = __get_free_pages(GFP_KERNEL,get_order(BUF_SIZE));
	bf_dev->rx_buf = __get_free_pages(GFP_KERNEL,get_order(BUF_SIZE));
#endif
