...
int32_t btl_core_matchFpPaul(uint8_t *fp_image);
#endif
...
uint32_t read_image_and_cal_dacp()
{
	static uint8_t *pimage = NULL;
	int ret  = -1;
	int device_fd = 0;
	...
	if(g_chip_id == 0x5383)
	{
		width = 80;
		height = 32;
	}else if(g_chip_id == 0x5483)
	{
		return 0;
	}
	LOGD("read_image_and_cal_dacp width=%d,height=%d",width,height);
	device_fd =open(deviceNode, O_RDWR);

	if(pimage == NULL)
		pimage = malloc(width * height);
		
	gFingerprintVar.dacp = g_def_capture_contrast;
    read(device_fd,pimage,(width * height));
    //setpixcol(pimage,"/data/system/users/0/fpdata/testimage.bmp");	
    REG_VALUE oldRegValue = {g_def_gain,0x48, gFingerprintVar.dacp};
	REG_VALUE newRegValue;
	ret = bl_AutoGainDacp(pimage, width , height,  &oldRegValue, &newRegValue,128,g_contrast_direction);
	if(ret == 0)
	{
		gFingerprintVar.dacp = newRegValue.reg_value_dacp;
...
static int32_t btl_core_captureImageWithAgc(
    ...
    int32_t type,   // 1 for match  2 for enroll  as save image
    int32_t *pre,   // whether has been preprocess
    ...
    fp_image_t* pFpImageOrg)
{
    ...
    uint8_t  pEnhanceData[g_width * g_height];    
    ...
    curContrast = gFingerprintVar.dacp;
    ...
    while (matchCompleted == 0) {
        ...
        if(isValidFpImage) {
			...
			//because of 3290 have black edge
			if(g_chip_id == 0x5183)
				ret = FP_ImagePreproc(imageA, g_width, g_height, 0, &(pFpImage->area_thre_shold), 0x02);
			else
				ret = FP_ImagePreproc(imageA, g_width, g_height, 0, &(pFpImage->area_thre_shold), 0);
			if(ret == 0)
			{
			...
			LOGD ("ret=%d mybestContrast=%x oldGainDacp.dacp=%x newGainDacp.dacp=%x",ret, mybestContrast,oldRegValue.reg_value_dacp, newRegValue.reg_value_dacp);
			}
			else
			{
				LOGD("autoGainDacp failed ret=%d",ret);
...
/*----------------------------------------------------------------------*/
void bl_get_pb_feedback_ilands(struct ilands_data *iland_feedback)
{
	LOGD("get_pb_feedback_ilands nbr_of_islands=%d ",iland_feedback->nbr_of_islands);
}
/*----------------------------------------------------------------------
purpose:
return :
...
int32_t btl_core_registeFp(int *pRadio)
{
    ...
    g_fd = tempFd;
	fpImage.area_thre_shold = ENROLL_AREA;
...
#if MATCH_BEFORE_ENROLL        
	ret = btl_core_matchFpPaul(fpImage.p_image);
	if (ret == DECISION_MATCH) {
        LOGE("%s,captureImage ret:%d", __func__,ret);
        return -ERR_EXIST;
    }
#endif
    ...
    if (Btl_Enroll(fpImage.p_image,g_width , g_height,0, 0,&g_schedule,&nConverage,NULL) < 0){
        ...
        {
          //if(pre == 0)
	      	btl_util_save_image(fpImage_org.p_image,2,gFingerprintVar.fid,g_schedule,fpImage_org.contrast,0,0);
		 //else
		  	btl_util_save_image(fpImage.p_image,2,gFingerprintVar.fid,g_schedule,fpImage.contrast,0,1);
        }
		#endif
		return -ERR_ENROLL;
	}
		...
		//if(pre == 0)
	      btl_util_save_image(fpImage_org.p_image,2,gFingerprintVar.fid,g_schedule,fpImage_org.contrast,1,0);
		// else
		 btl_util_save_image(fpImage.p_image,2,gFingerprintVar.fid,g_schedule,fpImage.contrast,1,1);
    ...
    int32_t *fingerID)
{
    ...
    LOGD("%s++",__func__);
    fpImage.p_image = gFingerprintVar.captured_image;
    btl_util_time_update(1);
    for (i = 0; i < nOpenTimes; i++) {
        if (g_CancelFlag)
            return -ERR_CANCEL;
	...
	fpImage.area_thre_shold = MATCH_AREA;
    ...
    for (i = 0; i < fid_size; i++) {
		...
		nTemplateSize[i] = gFingerprintVar.enrolled_template[i].fp_template_size;
 	}
    ...
    { 
		...
		//if(pre == 0)
			btl_util_save_image(fpImage_org.p_image,1,fid_index[nID],smatch_times,fpImage_org.contrast,1,0);	
		//	else
			btl_util_save_image(fpImage.p_image,1,fid_index[nID],smatch_times,fpImage.contrast,1,1);	
		
		} else{
		
	//	if(pre == 0)
			btl_util_save_image(fpImage_org.p_image,1,0,smatch_times,fpImage_org.contrast,0,0);
	//		else
			btl_util_save_image(fpImage.p_image,1,0,smatch_times,fpImage.contrast,0,1);
    ...
    if (nDecision == DECISION_MATCH) {
        *fingerID = fid_index[nID];
		gFingerprintVar.matched_index = nID;
		if(fpImage.org_area >= UPDATE_AREA){
			g_match_full = 1;
		}
        LOGD("nDecision : %d, ID: %d, %d",nDecision,*fingerID, fpImage.org_area);
    } else {
        LOGE("%s, nDecision : %d",__func__,nDecision);
    }

    btl_util_time_diffnow("match all time",1);
    return nDecision;
}
...
int32_t btl_core_matchFpPaul(uint8_t *fp_image)
{
    int32_t nDecision  = DECISION_NON_MATCH; 
   	int32_t nID = 0;
   	int32_t i = 0;
   	int32_t  uid = (int32_t)gFingerprintVar.gid;
    uint8_t* pTemplateBuffer[5];
	int32_t  nTemplateSize[5];
	int32_t fid_size = g_fid_index - 1;
	LOGD("fid_size=%d",fid_size);
    ...
    nDecision = Btl_MatchB(
                    "btl.db", // name of template's database
                    fp_image, // fp's image
                    g_width,
                    g_height,
                    g_far_value,    //  value of FAR
                    ...
                    pTemplateBuffer,     // buffer of template's data
                    nTemplateSize);       // size of template's data
	LOGD("nDecision : %d, ID: %d",nDecision,nID);
	return nDecision;
}
#endif
/*----------------------------------------------------------------------
...
int32_t btl_core_getDefaultParams()
{
	...
	g_contrast_direction = ST_PASSIVE;
	if(g_chip_id == 0x5183)
	{
		g_width = 112;
		g_height = 96;
		...
		g_update_far_value = BF_PB_FAR_100000;
		//if (btl_hw_get_chip_info(g_fd,&chip_info)  == 0){
		//	g_contrast_direction = (chip_info.chip_driver_type == 0) ? 0:1;
		//}
	...
	}else if(g_chip_id == 0x5383)
	{
		...
		g_height = 80;
		g_area_max = 15;
		g_best_algo = 37;//28;//20;		
		TEMPLATE_SIZE = 120 * 1024;//150 * 1024;//300 * 1024;
		g_far_value = BF_PB_FAR_50000;//BF_PB_FAR_10000;//BF_PB_FAR_20M;
		g_update_far_value = BF_PB_FAR_100000;//BF_PB_FAR_20K;//BF_PB_FAR_100M;
	}else if(g_chip_id == 0x5483)
	{
		g_width = 64;
		g_height = 80;
	...
	//Btl_SetIandFeedback(bl_get_pb_feedback_ilands);
    g_inited = 1;
    return 0;
...
int32_t btl_core_reportkey_ctl(int32_t enable)
{
    ...
    ret = ioctl(fd, BL229X_REPROTKEY_NEEDED, enable);
    if (ret == -1) {
		
        LOGE("%s set report key %d fail!\n error:%s",__func__, enable, strerror(errno));
        return -ERR_IO;
    }
    close(fd);
	return 0;
