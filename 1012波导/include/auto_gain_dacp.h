...
dstMean    [in]图像目标灰度均值
nSensorType    [in]模组类型，ST_PASSIVE表示被动式（如3290、3182、3390等），
		ST_ACTIVE表示主动式（如2390E、2180等），ST_JACK适用于BF3590和BF3582。
*/
LIB_API int bl_AutoGainDacp(unsigned char* imgdata, int width, int height, REG_VALUE* oldRegValue, REG_VALUE* newRegValue, unsigned char dstMean, int nSensorType);
