...
void btl_util_save_image(uint8_t*  image, int type , int fid ,int times ,int dacp , int ispass ,int ispre)
{
	...
	LOGD("%s: type :%d  fid:%d  times:%d  dacp:%d  ispass:%d  ispre:%d", __func__,type,fid,times,dacp,ispass,ispre);
		...
		if(type == 1) //match
			{
				if(ispass == 1)
					{  
						    ...
						    sprintf(imagename,"/data/system/users/0/fpdata/Verify_I(%02d)_N(%06d)_D(%03d)_P_E.bmp",fid,times,dacp);

							else

							sprintf(imagename,"/data/system/users/0/fpdata/Verify_I(%02d)_N(%06d)_D(%03d)_P.bmp",fid,times,dacp);
							...
							sprintf(imagename,"/data/system/users/0/fpdata/Verify_I(%02d)_N(%06d)_D(%03d)_F_E.bmp",fid,times,dacp);

							else
							
							sprintf(imagename,"/data/system/users/0/fpdata/Verify_I(%02d)_N(%06d)_D(%03d)_F.bmp",fid,times,dacp);
				
				}

								setpixcol(image,imagename);

				}else if(type == 2){//enroll
					...
					if(ispass== 1)
						...
						if(ispre == 1)
								...
								sprintf(imagename,"/data/system/users/0/fpdata/Enroll_I(%02d)_N(%02d)_D(%03d)_P_E.bmp",fid,times,dacp);
						
								else
						
								sprintf(imagename,"/data/system/users/0/fpdata/Enroll_I(%02d)_N(%02d)_D(%03d)_P.bmp",fid,times,dacp);
								...
								sprintf(imagename,"/data/system/users/0/fpdata/Enroll_I(%02d)_N(%02d)_D(%03d)_F_E.bmp",fid,times,dacp);
						
								else
								
								sprintf(imagename,"/data/system/users/0/fpdata/Enroll_I(%02d)_N(%02d)_D(%03d)_F.bmp",fid,times,dacp);
			}
						setpixcol(image,imagename);
