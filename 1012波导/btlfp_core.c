...
{
    int32_t  nIsDry = 0,nIntensity = 0,nImgVar = 0;
	...
	uint32_t tmp_area_thredshold = pFpImage->area_thre_shold;
    ...
    while (matchCompleted == 0) {
        ...
        if(isValidFpImage) {
			...
			pFpImage->area_thre_shold = tmp_area_thredshold;
			if(g_chip_id == 0x5183)
...
int32_t btl_core_isFingerUp ()
{
    ...
    while (g_CancelFlag != 1 && timeoutCouns < maxTimeOuts) {
        ...
        if (imageCheckRet == -1) {
            validImageCounts = 0;
            emptyImageCounts ++;
            if (emptyImageCounts >= checkFingerUpMaxCounts) {   
                fingerUpFlag = 1;
                break;
            }
        } else {
