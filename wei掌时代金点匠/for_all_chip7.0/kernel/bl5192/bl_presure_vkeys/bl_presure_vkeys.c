...
void fwupdate_func(struct work_struct *work)
{	
	int error = -1;
...
DECLARE_DELAYED_WORK(fwupdate_delayed_work, fwupdate_func);
#endif
...
int bl_ts_i2c_transfer_dma(__u8 i2c_addr, __u8 *buf, __u16 len,__u8 rw)
{
   ...
   if(rw){
	    ...
	    if (ret < 0)
	    {
	    	BTL_DEBUG("___i2c DMA transfer error___(%d)\n", ret);
	        return -1;
	    }
...
static int bl_apk_open(struct inode *inode, struct file *file)
{
	...
	if (apk_data == NULL) {	
		BTL_DEBUG("can't allocate memory for apk\n");
		return -1;
	}
	return 0;
}
...
static ssize_t bl_apk_write(struct file *file, const char __user *buff, size_t count, loff_t *offp)
{
	...
	switch(apk_data[0]){
		...
		case WRITE_CMD :{
			BTL_DEBUG("WRITE_CMD");
			ret = bl_i2c_transfer(apk_data[1], &apk_data[FRAME_CMD_LEN], 
				len,I2C_WRITE);
			if(ret < 0){
				BTL_DEBUG("___%s:i2c transfer error___\n",__func__);
				return -1;
			}
			BTL_DEBUG();
		}break;
...
static ssize_t bl_apk_read(struct file *file, char __user *buff, size_t count, loff_t *offp)
{
	...
	switch(apk_data[0]){
		...
		case READ_CMD :{
			BTL_DEBUG("READ_CMD");
			ret = bl_i2c_transfer(apk_data[1], apk_data, len,I2C_READ);
			if(ret < 0){
				printk("___%s %d:i2c transfer \n",__func__,__LINE__);
				return -1;
			}
			BTL_DEBUG();
		}break;
...
#if AUTO_UPDATE_FIRMWARE
	schedule_delayed_work(&fwupdate_delayed_work, 0);		
#endif

	/*for debug apk*/

	g_bl_pdata.proc_entry = proc_create("Betterlife_ts", 0666, NULL,&bl_apk_fops);
#if SUPPORT_I2C_DMA
	gpDMABuf_va = (u8 *) dma_alloc_coherent(&g_bl_pdata.client->dev, 250,
		&gpDMABuf_pa, GFP_KERNEL);
	if (!gpDMABuf_va) {
		BTL_DEBUG("Allocate DMA I2C Buffer failed!");
		return -1;
	}
	memset(gpDMABuf_va, 0, 250);
#endif
    return 0;
}
...
int  bl_remove(struct i2c_client *client)
{
	if ( g_bl_pdata.proc_entry != NULL ){
		remove_proc_entry(PRESURE_DRIVER_NAME, NULL);
	}
	return 0;
}
