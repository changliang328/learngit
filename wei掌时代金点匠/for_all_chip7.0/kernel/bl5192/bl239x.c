...
#define PLATFORM_POWER_EN (0)
#define BL_QUIK_WK_CPU_EN (0)
#define NAVI_FUNC_EN (1)
...
#define BL229X_WAIT_FINGER_UP	         _IOWR(BL229X_IOCTL_MAGIC_NO, 16, u32)
...
#define FP_DET_FINGER_DOWN             (0)
#define FP_DET_FINGER_UP           (1)

#define INT_MO
#define FP_DETECT_METHOD        (FP_DET_FINGER_DOWN ) //0 FD:mode, 1: NAV MODE
...
// sensor parameter
#define FINGER_TD_THRED_HIGH_VALUE		(0x6)
...
#if BL_QUIK_WK_CPU_EN
//add for open core begin
#include <linux/workqueue.h>
#include <linux/cpu.h>
struct work_struct core_work;
struct workqueue_struct* core_queue;
...
{
	int cpu;
	BTL_DEBUG("betterlife:Entry work_handler");
...
{
	core_queue = create_singlethread_workqueue("bl_wk_main"); //cretae a signal thread worker queue
...
static struct mt_chip_conf spi_conf= {
	...
	.setuptime = 10,
	.holdtime = 10,
	.high_time = 10, //\B4˴\A6\BE\F6\B6\A8slk\B5\C4Ƶ\C2\CA
	.low_time =  10,
	.cs_idletime = 20, //10,
	//.ulthgh_thrsh = 0,
...
static int	bl229x_dev_init_3590(struct bl229x_data *bl229x);
static int  bl229x_dev_capture_init(struct bl229x_data *bl229x,u32 arg);
static int  bl229x_dev_interrupt_init(struct bl229x_data *bl229x, int downup);
...
/*----------------------------------------------------------------------------*/
static void early_suspend(struct bl229x_data *bl229x)
{
	bl229x->is_early_suspend = 1;
	if(g_bl229x->is_matching == 1)
	{
...
static void early_resume(struct bl229x_data *bl229x)
{
	bl229x->is_early_suspend = 0;
	if(g_bl229x->is_matching == 1)
	{
	}
	else
		...
		unsigned long event, void *data)
{
...

	{
		...
		{
			early_resume(g_bl229x);
		}
		else if (*blank == FB_BLANK_POWERDOWN)
		{
			early_suspend(g_bl229x);
		}
	}
...
static int spi_read_frame(struct bl229x_data *bl229x)
{
	...
	BTL_DEBUG("%s,BL229X_WIDTH=%d,BL229X_HEIGHT=%d,DMA_TRANSFER_SIZE=%d,READIMAGE_BUF_SIZE=%d++\n",__func__,BL229X_WIDTH,BL229X_HEIGHT,DMA_TRANSFER_SIZE,READIMAGE_BUF_SIZE);
	...
	bl229x_spi_write_reg(REGA_HOST_CMD, MODE_PIEXL_TEST);
	...
	}else if(fingerprintdParams.chip_id == 0x5483)
	{
		memcpy(bl229x->image_buf,spi_rx_local_buf + OMIT_PIXCEL, bl229x_image_size);
		exchange_odd_even_cols(bl229x->image_buf, BL229X_WIDTH,BL229X_HEIGHT);
	}else
...
static int spi_read_frame(struct bl229x_data *bl229x)
{
	...
	{
		memcpy(bl229x->image_buf,bl229x->imagerxpix + OMIT_PIXCEL, bl229x_image_size);
		exchange_odd_even_cols(bl229x->image_buf, BL229X_WIDTH,BL229X_HEIGHT);
	}
	else
		memcpy(bl229x->image_buf,bl229x->imagerxpix + OMIT_PIXCEL, bl229x_image_size);
...
static int  bl229x_dev_selftest(struct bl229x_data *bl229x)
{
	...
	if((chip_id == 0x5283)||(chip_id == 0x5383)||(chip_id == 0x5483))
	{
...
static int bl229x_read_chipid1(struct bl229x_data *bl229x)
{
	...
	}else if(id == 0x5383)//3390
	{
		...
		SENSOR_DEFAULT_CONTRAST_V3 = 0xaf;
	...
	}else if(id == 0x5483)
	{
		BL229X_WIDTH = 64;
		BL229X_HEIGHT = 80;	
		SENSOR_DEFAULT_GAIN_V3 = 0x6e;
		SENSOR_CAPTURE_GAIN_V3 = 0x6e;
		SENSOR_DEFAULT_CONTRAST_V3 = 0x90;
		bl229x_dev_init = bl229x_dev_init_3590;
		bl229x_read_chipid = bl229x_read_chipid1;
		REGA_FINGER_DT_INTERVAL_LOW = 0x25;
		REGA_FINGER_DT_INTERVAL_HIGH = 0x26;
		REGA_HOST_CMD = 0x34;
...
/*----------------------------------------------------------------------------*/
 int bl229x_reset_for_esd(struct bl229x_data *bl229x)
{
...
static int bl229x_dev_init_3590(struct bl229x_data *bl229x)
{
	...
	bl229x_spi_write_reg(0x2b,0x38);
	bl229x_spi_write_reg(0x10,0x58);
	bl229x_spi_write_reg(0x11,0x0);
	bl229x_spi_write_reg(0x12,0x6);
	bl229x_spi_write_reg(0x25,0x0);
	bl229x_spi_write_reg(0x26,0xa);
	bl229x_spi_write_reg(0x1b,0x8c);
	bl229x_spi_write_reg(0x1c,0x8c);
	bl229x_spi_write_reg(0x1d,0x90);
	bl229x_spi_write_reg(0x1e,0x90);
	bl229x_spi_write_reg(0x31,0xff);
	bl229x_spi_write_reg(0x32,0x33);
	bl229x_spi_write_reg(0x17,0x1);
	bl229x_spi_write_reg(0x2d,0xe1);
	bl229x_spi_write_reg(0x39,0xb3);
	return 0;
}
...
static int  bl229x_dev_interrupt_init(struct bl229x_data *bl229x, int downup)
{
	...
	else if(fingerprintdParams.chip_id == 0x5383)
	{
		...
		if(g_bl229x->is_matching == 1)
		{
			row_start = 24;
			row_len = 32;
			bl229x_spi_write_reg(RRG_FRAME_ROW_START, row_start & 0xff);
			bl229x_spi_write_reg(RRG_FRAME_ROW_LEN, row_len & 0xff);
			BL229X_WIDTH = 80;
			BL229X_HEIGHT = row_len;	
			BTL_DEBUG("BL229X_WIDTH=%d,BL229X_HEIGHT=%d",BL229X_WIDTH,BL229X_HEIGHT);
		}else
		{
			row_start = 1;
			row_len = 80;
	...
	if(frame_num != 0)
	{
		bl229x_spi_write_reg(0x2b, frame_num);
	}else
	{
		bl229x_spi_write_reg(0x17, 0x2c);
		bl229x_spi_write_reg(0x0d, 0x3);
		bl229x_spi_write_reg(0x17, 0xac);
	}
	if(fingerprintdParams.chip_id != 0x5183)
	{
	 	if(downup == FP_DET_FINGER_DOWN)
			bl229x_spi_write_reg_bit(0x10,2,0);
	 	else
			bl229x_spi_write_reg_bit(0x10,2,1);
	}
	...
	bl229x_spi_write_reg(cap_dacp_reg, bl229x->chip_params.m_contrast);
	bl229x->interrupt_mode = INT_MODE_KEY;
...
static int bl229x_dev_capture_init(struct bl229x_data *bl229x,u32 arg)
{
	...
	{
		bl229x_spi_write_reg(0x17, 0x2c);
		bl229x_spi_write_reg(0x0d, 0x1b);
		bl229x_spi_write_reg(0x17, 0xac);
	}
	g_bl229x->is_frame_ready = 0;
	bl229x->interrupt_mode = INT_MODE_CAPTURE;
	bl229x_spi_write_reg(cap_dacp_reg, bl229x->contrast);
	...
	switch (testType){
		case 1:  // mode switch
			...
			if (workmode == MODE_IDLE)
				...
				bl229x_dev_interrupt_init(bl229x,FP_DET_FINGER_DOWN );
			else if (workmode == MODE_FG_CAP)
				bl229x_dev_capture_init(bl229x,10);
			else if (workmode == MODE_NAVI)
				bl229x_dev_interrupt_init(bl229x,FP_DET_FINGER_UP);
			break;
		case 2:  // init action 
			if (params[1] == 1)
...
#define PRESS_UP_COUNT (200)
#define PRESS_DOWN_COUNT (2000)
...
static int g_is_pressed = 0;
static int g_first_enter_key = 0;

int cal_touch_area_point(struct bl229x_data *bl229x,int is_navi_mode)
{
	...
	if(area_count < PRESS_UP_COUNT)
	{
		...
		return -1;
	}else if(area_count > PRESS_DOWN_COUNT){
		g_is_pressed = 1;
		if(is_navi_mode == 0)
			return 0;
	}
	if(touch_index == 0)
	{
		...
		start_tmp.meanvalue = touch_area[touch_index].meanvalue;
	}	
	else
	{
...
static int  bl229x_dev_reset(struct bl229x_data *bl229x)
{
	...
	//test reset pin
	hw_reset(bl229x);
	if((fingerprintdParams.chip_id == 0x5283)||(fingerprintdParams.chip_id == 0x5383)||(fingerprintdParams.chip_id == 0x5483))
	{//\C7\E5\B3\FD\B8\B4λ\D6ж\CF
		bl229x_spi_write_reg(0x13,0x40);
		bl229x_spi_write_reg(0x13,0);
	}
...
/*----------------------------------------------------------------------------*/
int bl_navi_handler(void)
{
	u8 intStatus = 2;
	int ret = 0;
	int is_navi_mode = 0;// 0 for key, 1 for navi
	BTL_DEBUG("is_busy=%x is_navi_mode=%d++++\n",atomic_read(&is_busy),is_navi_mode);
	if((g_bl229x->need_report == 1)&&(g_bl229x->is_early_suspend == 0)){
		touch_index = 0;
		mod_timer(&g_bl229x->key_timer,jiffies + 10*HZ);
		g_first_enter_key = 1;
		g_is_pressed = 0;
		gDirection = 0;
		while(1)
		{
			if (getSensorInterruptStatus(g_bl229x) != intStatus) {
				...
				ret = cal_touch_area_point(g_bl229x, is_navi_mode);
				if(g_first_enter_key == 1)
				{
					if(g_is_pressed == 1)
					{
						if(is_navi_mode == 0)
				...
				BTL_DEBUG("isfingerup ret = %d,is_busy=%d\n", ret,atomic_read(&is_busy));									
			}
			if((atomic_read(&is_busy) == 1)&&(atomic_read(&g_bl229x->report_timeout) == 1))
			{
				atomic_set(&g_bl229x->report_timeout,0);
				break;
			}
			if(g_is_pressed == 0)
				break;
	...
	}else if(g_bl229x->is_early_suspend == 1 && g_bl229x->need_report == 2){
		BTL_DEBUG("is_frame_ready:%d\n", g_bl229x->is_frame_ready);
		if(fingerprintdParams.chip_id != 0x5483)
		{
			spi_read_frame(g_bl229x);
...
irqreturn_t bl229x_eint_handler(int irq,void *data)
{
...
#endif
	BTL_DEBUG("  ++\n");
	if(atomic_read(&finger_up) == 0)
	{
		atomic_set(&finger_up, 1);
		wake_up_interruptible(&frame_waiter);
		return IRQ_HANDLED;
	}
	...
	if(fingerprintdParams.chip_id == 0x5483)
	{
		...
		intStatus = 1;
		else if (g_bl229x->interrupt_mode == INT_MODE_CAPTURE) 
		intStatus = 2;
	}
		BTL_DEBUG("mode:%d intStatus=%d\n",g_bl229x->interrupt_mode,intStatus);
...
static long bl229x_ioctl(struct file *filp, unsigned int cmd,unsigned long arg)
{
	...
	switch (cmd) {
		...
		case BL229X_REPORTKEY_NEEDED:
			...
			{
				g_bl229x->is_matching = 1;
			}
			else
			{
				g_bl229x->is_matching = 0;
			}
			BTL_DEBUG("g_bl229x->need_report = %d", g_bl229x->need_report);
			break;
		case BL229X_WAIT_FINGER_UP:
			atomic_set(&finger_up, 0);
			BTL_DEBUG("finger_up = %d", atomic_read(&finger_up));
			bl229x_dev_interrupt_init(bl229x, FP_DET_FINGER_UP);
			error = wait_event_interruptible_timeout(frame_waiter,atomic_read(&finger_up), 5*HZ);
			BTL_DEBUG("finger_up = %d ,error=%d", atomic_read(&finger_up),error);
			atomic_set(&finger_up, 1);
			break;
		default:
			error = -ENOTTY;
...
{
    struct bl229x_data *bl229x = (struct bl229x_data *)data;
    BTL_DEBUG("++");
	atomic_set(&bl229x->report_timeout,1);
}
static int  bl229x_probe(struct spi_device *spi)
{
	...
	atomic_set(&bl229x->report_timeout, 0);
	bl229x->reset = 0;
	bl229x->need_report = 1;
	bl229x->is_matching = 0;
	bl229x->is_early_suspend = 0;
	...
	mtspi_set_dma_en(0);
   	...
   	if(bl229x_read_chipid1(bl229x) != 0x5483 && bl229x_read_chipid1(bl229x) != 0x5383 && bl229x_read_chipid1(bl229x) != 0x5283)
   	{
...
#endif
	init_timer(&bl229x->key_timer);
	bl229x->key_timer.expires = jiffies + 10*HZ; 
	bl229x->key_timer.data = (unsigned long)bl229x;     
	bl229x->key_timer.function = bl_key_timer_function; 
