...
{
	BTL_DEBUG("++ is_matching = %d", g_bl229x->is_matching);
	g_bl229x->is_early_suspend = 1;
...
static int bl229x_read_chipid1(struct bl229x_data *bl229x)
{
	...
	if(id == 0x5283)//3182
	{
		BL229X_WIDTH = 72;
		BL229X_HEIGHT = 128;
		SENSOR_DEFAULT_GAIN_V3 = 0x2c;  //   //0x2c...
		SENSOR_CAPTURE_GAIN_V3 = 0x2c;   //
		SENSOR_DEFAULT_CONTRAST_V3 = 0x78;  //0x78
	...
	}else if(id == 0x5383)//3390
	{
		BL229X_WIDTH = 80;
		BL229X_HEIGHT = 80;	
		SENSOR_DEFAULT_GAIN_V3 = 0x38;
		SENSOR_CAPTURE_GAIN_V3 = 0x38;
		SENSOR_DEFAULT_CONTRAST_V3 = 0x96;
...
static int bl229x_read_chipid2(struct bl229x_data *bl229x)
{
	...
	if(chip_id == 0x5183)
	{
		BL229X_WIDTH = 112;
		BL229X_HEIGHT = 96;	
		SENSOR_DEFAULT_GAIN_V3 = 0x35;  
		SENSOR_CAPTURE_GAIN_V3 = 0x35;   
...
#define FINGER_SPI_SCK_PIN              (GPIO81 | 0x80000000)
#define FINGER_SPI_SCK_PIN_M_SPI_CK     GPIO_MODE_01
#define	FINGER_SPI_CS_PIN               (GPIO80 | 0x80000000)
#define	FINGER_SPI_CS_PIN_M_SPI_CS      GPIO_MODE_01
#define	FINGER_SPI_MOSI_PIN             (GPIO83 | 0x80000000)
#define	FINGER_SPI_MOSI_PIN_M_SPI_MOSI  GPIO_MODE_01
#define	FINGER_SPI_MISO_PIN             (GPIO82 | 0x80000000)
#define	FINGER_SPI_MISO_PIN_M_SPI_MISO  GPIO_MODE_01

#define GPIO_FP_POWER_PIN   GPIO63
#define	GPIO_FP_POWER_PIN_M_GPIO	GPIO_MODE_00



#define	FINGER_RESET_PIN                (GPIO168 | 0x80000000)
#define	FINGER_RESET_PIN_M_GPIO         GPIO_MODE_00

#define FINGER_EINT_GPIO              (GPIO19 | 0x80000000)  // (GPIO84 | 0x80000000) // (GPIO20 | 0x80000000)
#define FINGER_EINT_GPIO_MODE          GPIO_MODE_00
#define FINGER_EINT_NUM                19  
#define FINGER_EINT_TYPE            CUST_EINTF_TRIGGER_RISING
...
#if(MTK_ANDROID_L == 1)
	bl229x->irq_gpio = FINGER_EINT_GPIO;
	bl229x->reset_gpio = FINGER_RESET_PIN;
	bl229x->irq_num = FINGER_EINT_NUM;
	bl229x->power_en_gpio = GPIO_FP_POWER_PIN;
#else
...
err:
	...
	mt_set_gpio_mode(FINGER_SPI_CS_PIN, FINGER_SPI_CS_PIN_M_SPI_CS);
	mt_set_gpio_pull_enable(FINGER_SPI_CS_PIN, GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(FINGER_SPI_CS_PIN, GPIO_PULL_UP);

	mt_set_gpio_mode(FINGER_SPI_SCK_PIN, FINGER_SPI_SCK_PIN_M_SPI_CK);
	mt_set_gpio_pull_enable(FINGER_SPI_SCK_PIN, GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(FINGER_SPI_SCK_PIN, GPIO_PULL_DOWN);

	mt_set_gpio_mode(FINGER_SPI_MISO_PIN, FINGER_SPI_MISO_PIN_M_SPI_MISO);
	mt_set_gpio_pull_enable(FINGER_SPI_MISO_PIN, GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(FINGER_SPI_MISO_PIN, GPIO_PULL_DOWN);

	mt_set_gpio_mode(FINGER_SPI_MOSI_PIN, FINGER_SPI_MOSI_PIN_M_SPI_MOSI);
	mt_set_gpio_pull_enable(FINGER_SPI_MOSI_PIN, GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(FINGER_SPI_MOSI_PIN, GPIO_PULL_DOWN);

	mt_set_gpio_mode(FINGER_RESET_PIN, FINGER_RESET_PIN_M_GPIO);
	mt_set_gpio_dir(FINGER_RESET_PIN, GPIO_DIR_OUT);

	mt_set_gpio_mode(FINGER_EINT_GPIO, FINGER_EINT_GPIO_MODE);
	mt_set_gpio_dir(FINGER_EINT_GPIO, GPIO_DIR_IN);
	mt_set_gpio_pull_enable(FINGER_EINT_GPIO, GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(FINGER_EINT_GPIO, GPIO_PULL_UP);
...
{	
	...
	if(j < PRESS_UP_COUNT)
	{
		g_is_pressed = 0;
		ret = 1;
	}else if(j > PRESS_DOWN_COUNT)
	{
		g_is_pressed = 1;
	}
	BTL_DEBUG("black count: %d is_pressed=%d", j, g_is_pressed);
	return ret;
}
/*----------------------------------------------------------------------------*/
irqreturn_t bl229x_eint_handler(int irq,void *data)
...
static int  bl229x_probe(struct spi_device *spi)
{
	...
	bl229x->reset = 0;
	bl229x->need_report = 2;
	bl229x->is_matching = 1;
	...
	bl229x->irq_num = 19;  //=FINGER_EINT_NUM
...
#ifdef BTL_USE_WORKQUEUE
	mt_eint_registration(bl229x->irq_num, FINGER_EINT_TYPE,bl229x_eint_handler_l,0);
	...
	if (err)
		dev_err(&bl229x->spi->dev, "Unable to register fb_notifier: %d\n",
				err);
	#else
	bl229x->early_suspend.suspend = early_suspend,
	bl229x->early_suspend.resume = early_resume,
