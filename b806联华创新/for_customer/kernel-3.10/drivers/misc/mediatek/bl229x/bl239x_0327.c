...
static int bl229x_parse_dt_gpio(struct device *dev,struct bl229x_data *pdata)
{
    ...
    if (of_property_read_u32_index(dev->of_node, "fingerprint,rst-gpio", 0, &pdata->reset_gpio))
    {
        BTL_DEBUG("fingerprint,rst-gpio get pin fail!!!\n");
    }
    if (!gpio_is_valid(pdata->reset_gpio))
        return -EINVAL;

    if (of_property_read_u32_index(dev->of_node, "fingerprint,touch-int-gpio", 0, &pdata->irq_gpio))
    {
        BTL_DEBUG("fingerprint,touch-int-gpio get pin fail!!!\n");
    }
    if (!gpio_is_valid(pdata->irq_gpio))
        return -EINVAL;

    if (of_property_read_u32_index(dev->of_node, "fingerprint,en-gpio", 0, &pdata->power_en_gpio))
    {
        BTL_DEBUG("fingerprint,en-gpio get pin fail!!!\n");
    }
