...
public class Activity extends ContextThemeWrapper
    ...
    static final String FRAGMENTS_TAG = "android:fragments";
    	...
    	//blestech add
	public static String mlastPackage = "com.btlfinger.fingerprintunlock";
	private BroadcastReceiver receiver = new BroadcastReceiver() {

        @Override
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals("com.blestech.fingerprint.lockui")) {
            	mlastPackage = "com.btlfinger.fingerprintunlock";
				unRegisterReceiver();
            }
        }
    };
     ...
     * @hide
     */
    public void startAct (String packageName,String className,String extraPackageString) {
    ...
    protected void onResume() {
        ...
        if (true) {
            ...
            String lastPackage = Settings.System.getString(getContentResolver(), Settings.System.BTLFINGER_FINGERPRINT_LAST_PACKAGE_NAME);
            int appLockAble = Settings.System.getInt(getContentResolver(), Settings.System.BTLFINGER_FINGERPRINT_USEDTO_APPLOCK, 0);

            Log.e("btlfinger","packagenameString-->" + packagenameString + " | lastPackage-->" + lastPackage);

            if (appLockAble == 1 && !packagenameString.equals("com.btlfinger.fingerprintunlock") 
                && (mIntent != null && !mIntent.getComponent().getClassName().equals("com.android.phone.EmergencyDialer"))/*紧急拨号无需加密*/
                && lastPackage != null && !lastPackage.equals(packagenameString)
                && !mIntent.getComponent().getClassName().equals("com.android.settings.aliyun.UsbStorageActivity")
				&& !mIntent.getComponent().getClassName().equals("com.android.email.activity.Welcome")
				|| (appLockAble == 1 && !packagenameString.equals("com.btlfinger.fingerprintunlock") 
				&&lastPackage != null && !lastPackage.equals(packagenameString) 
                && packagenameString.equals("com.google.android.googlequicksearchbox"))) {
                
                String appString = Settings.System.getString(getContentResolver(), Settings.System.BTLFINGER_FINGERPRINT_NEEDLOCKAPP_PACKAGE_NAME);
                
                if (appString != null && !packagenameString.equals(mlastPackage)) {
		
            	
	                String[] appsStrings = appString.split("\\|");
                    for (int i = 0; i < appsStrings.length ;i++) {                 
                        if (packagenameString.equals(appsStrings[i])) {
            
                            Log.e("btlfinger"," need lockUI app:"+ packagenameString);
			    IntentFilter filter = new IntentFilter("com.blestech.fingerprint.lockui");
			    ...
			    this.registerReceiver(receiver, filter);
                            ...
                            if (isScreenLocked) {
                                ...
                                if (Settings.System.getInt(getContentResolver(), Settings.System.BTLFINGER_FINGERPRINT_USEDTO_SCREENLOCK, 0) == 1) {
                                    Log.i("btlfinger", "case 0:the lock has not unlocked, do not show the lockUI");
                                    Intent intent = new Intent("com.btlfinger.fingerprint.update.last_package_name");  
                                    intent.putExtra("packagename", packagenameString);
                                    sendBroadcast(intent);
                                } else {
 
                                    startAct("com.btlfinger.fingerprintunlock","com.btlfinger.fingerprintunlock.applock.LockUI", packagenameString);
				    mlastPackage = packagenameString;
				    break;
                                }
                            } else {
