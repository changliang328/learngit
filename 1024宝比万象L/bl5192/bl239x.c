...
static int bl229x_read_chipid1(struct bl229x_data *bl229x)
{
	...
	if(id == 0x5283)//3182
	{
		BL229X_WIDTH = 72;
		BL229X_HEIGHT = 128;
		SENSOR_DEFAULT_GAIN_V3 = 0x77;
		SENSOR_CAPTURE_GAIN_V3 = 0x77;
		SENSOR_DEFAULT_CONTRAST_V3 = 0x64;
...
static int bl229x_read_chipid2(struct bl229x_data *bl229x)
{
	...
	if(chip_id == 0x5183)
	{
		BL229X_WIDTH = 112;
		BL229X_HEIGHT = 96;	
		SENSOR_DEFAULT_GAIN_V3 = 0x1d;
		SENSOR_CAPTURE_GAIN_V3 = 0x1d;
		SENSOR_DEFAULT_CONTRAST_V3 = 0x55;
...
#define GPIO_FINGER_PRINT_RESET     (GPIO20 | 0x80000000)
...
#define GPIO_FP_POWER_PIN   (GPIO124 | 0x80000000)
...
#if(MTK_ANDROID_L == 1)
	bl229x->irq_gpio = GPIO_FP_INT_PIN;//GPIO_FP_INT_PIN ;
	bl229x->reset_gpio = GPIO_FINGER_PRINT_RESET;//GPIO_FINGER_PRINT_RESET ;
...
err:
	...
	mt_set_gpio_mode(GPIO_FINGER_PRINT_RESET, FINGER_RESET_PIN_M_GPIO);
...
int is_connected(struct bl229x_data *bl229x)
{
	...
	bl229x_spi_write_reg(REGA_HOST_CMD, MODE_IDLE);
	bl229x_spi_write_reg(REGA_RX_DACP_LOW,cur_contrast);
	printk("write %d",cur_contrast);
	m_is_conneted = bl229x_spi_read_reg(REGA_RX_DACP_LOW);
	printk("readback %d",m_is_conneted);
	if(cur_contrast == m_is_conneted)
	{
		printk("ok %d",bl229x_spi_read_reg(REGA_RX_DACP_LOW));
		return 1;
	}
	printk("failed %d",bl229x_spi_read_reg(REGA_RX_DACP_LOW));
	return 0;
}
...
static int  bl229x_probe(struct spi_device *spi)
{
	...
	BTL_DEBUG("  ++\n\n");
	printk("enter in bl229x_probe\n");
	bl229x = kzalloc(sizeof(struct bl229x_data),GFP_KERNEL);
	if (!bl229x) {
		printk("kzalloc bl229x error\n");
		return -ENOMEM;
	}
	g_bl229x = bl229x;
	...
	mtspi_set_dma_en(0);
   	...
   	{
   		printk("chip is not 3390 & 3182");
		if(bl229x_read_chipid2(bl229x) != 0x5183)
		{
			printk("chip is not 3290");
...
static int  mt_spi_init(void)
{
	...
	printk("spi_register_board_info ret=%d\n",ret);
#endif
	ret=spi_register_driver(&bl229x_driver);
	printk("spi_register_driver ret=%d\n",ret);
